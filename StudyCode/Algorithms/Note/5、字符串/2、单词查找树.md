...
将以上算法用的是 sting，也可以改为处理 Alphabet 
...
> 在单词查找树种查找或插入一个键，访问数组的次数最多是键的长度加一。
...
在三向单词查找树种，每个结点都含有一个字符、三条链接和一个值。这三条链接分别对应键的当前字符小于结点字符、等于结点字符、大于结点字符的所有键。

#### 查找和插入操作

在查找时，首先比较键的首字母和根结点的字母。如果键的首字母较小，则选择左链接。如果相等则选择中链接。如果键的首字母较大，则选择右链接。然后递归地使用相同的算法依次比较接下来的字母。如果遇到了一个空链接或者键结束时值为空，则查找失败。如果键结束时值非空，则查找命中。在插入时，首先进行查找，然后补全键末尾的所有结点。

> 键所对应结点在单词树种的层级大于或等于键的字符数。因为如果首字母小于根结点的字母，会进入到下一层级（根结点的左子结点）寻找结点字母等于首字母的结点。
...
三向单词查找树最重要的性质就是每个结点只含有三个链接，所以三向单词查找树所需的空间远小于对应的单词查找树。
...
在三向单词查找树中，需要用在二叉查找树中删除结点的方法来删除与该字符对应的结点
...
如果空间足够，R向单词查找树的速度是最快的。如果空间不足，三向单词查找树是最佳选择。